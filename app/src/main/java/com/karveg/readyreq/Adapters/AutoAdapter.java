...
public class AutoAdapter extends RecyclerView.Adapter<AutoAdapter.ViewHolder> {
    ...
    public AutoAdapter(List<Generic> objects, int id, int layout, Context context, int mode, AlertDialog progressDialog) {
    ...
    @Override
    public AutoAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(layout, parent, false);
        AutoAdapter.ViewHolder vh = new AutoAdapter.ViewHolder(v);
        return vh;
    }
        ...
        private final PopupMenu.OnMenuItemClickListener optionMenu = new PopupMenu.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.delItem:
                        Utils.showCtxDelete(objects, getAdapterPosition(), id, ctx, mode, MyApplication.AUTH, progressDialog);
                        break;
                }
                return true;
            }
