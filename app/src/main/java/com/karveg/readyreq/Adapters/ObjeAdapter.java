...
public class ObjeAdapter extends RecyclerView.Adapter<ObjeAdapter.ViewHolder> {
    ...
    public ObjeAdapter(List<Generic> objects, int id, int layout, Context context, int mode, AlertDialog progressDialog) {
    ...
    @Override
    public ObjeAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(layout, parent, false);
        ObjeAdapter.ViewHolder vh = new ObjeAdapter.ViewHolder(v);
        return vh;
    }

    //solo se ejecuta al crear
    @Override
    public void onBindViewHolder(ObjeAdapter.ViewHolder holder, int position) {
        ...
        private final PopupMenu.OnMenuItemClickListener optionMenu = new PopupMenu.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.delItem:
                        Utils.showCtxDelete(objects, getAdapterPosition(), id, ctx, mode, MyApplication.OBJE, progressDialog);
                        break;
                }
                return true;
            }
