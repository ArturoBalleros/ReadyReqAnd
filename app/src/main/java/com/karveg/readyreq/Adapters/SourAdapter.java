...
public class SourAdapter extends RecyclerView.Adapter<SourAdapter.ViewHolder> {
    ...
    public SourAdapter(List<Generic> objects, int id, int layout, Context context, int mode, AlertDialog progressDialog) {
    ...
    @Override
    public SourAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(layout, parent, false);
        SourAdapter.ViewHolder vh = new SourAdapter.ViewHolder(v);
        return vh;
    }

    //solo se ejecuta al crear
    @Override
    public void onBindViewHolder(SourAdapter.ViewHolder holder, int position) {
        ...
        private final PopupMenu.OnMenuItemClickListener optionMenu = new PopupMenu.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.delItem:
                        Utils.showCtxDelete(objects, getAdapterPosition(), id, ctx, mode, MyApplication.SOUR, progressDialog);
                        break;
                }
                return true;
            }
